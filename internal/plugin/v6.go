...
import (
	...
	proto "github.com/Perruer/unclick/internal/tfplugin6"
)
...
var caps6 = &proto.ClientCapabilities{}

func (p *v6) getSchema(ctx context.Context) (*schema.Provider, error) {
	resp, err := p.client.GetProviderSchema(ctx, &proto.GetProviderSchema_Request{})
	if err != nil {
		return nil, fmt.Errorf("GetProviderSchema: %w", err)
	}
	if err := diagError("GetProviderSchema", diags6(resp.Diagnostics)); err != nil {
		return nil, err
	}
	out := &schema.Provider{
	...
	if out.Provider, err = block6(resp.Provider.GetBlock()); err != nil {
...
func (p *v6) configure(ctx context.Context, config cty.Value, ty cty.Type) error {
	raw, err := encode(config, ty)
	if err != nil {
		return fmt.Errorf("encoding provider configuration: %w", err)
	}
	resp, err := p.client.ConfigureProvider(ctx, &proto.ConfigureProvider_Request{
	...
	return diagError("ConfigureProvider", diags6(resp.Diagnostics))
}

func (p *v6) readResource(ctx context.Context, typeName string, state cty.Value, ty cty.Type, private []byte) (cty.Value, []byte, error) {
	...
	if err := diagError("ReadResource "+typeName, diags6(resp.Diagnostics)); err != nil {
...
func (p *v6) importResourceState(ctx context.Context, typeName, id string, types func(string) (cty.Type, bool)) ([]ImportedResource, error) {
	...
	if err := diagError("ImportResourceState "+typeName, diags6(resp.Diagnostics)); err != nil {
		return nil, err
	}
...
func block6(b *proto.Schema_Block) (*schema.Block, error) {
	out := &schema.Block{
	...
	for _, a := range b.Attributes {
		attr, err := attribute6(a)
		if err != nil {
			return nil, fmt.Errorf("%s: %w", a.Name, err)
		}
		out.Attributes[a.Name] = attr
	}
	for _, nb := range b.BlockTypes {
		inner, err := block6(nb.Block)
		if err != nil {
			return nil, fmt.Errorf("%s: %w", nb.TypeName, err)
		}
		nesting, err := nesting6(nb.Nesting)
		if err != nil {
			return nil, fmt.Errorf("%s: %w", nb.TypeName, err)
		}
...
func attribute6(a *proto.Schema_Attribute) (*schema.Attribute, error) {
...
func object6(o *proto.Schema_Object) (*schema.Object, error) {
	out := &schema.Object{Attributes: map[string]*schema.Attribute{}}
	...
	case proto.Schema_Object_SINGLE:
		out.Nesting = schema.NestingSingle
	case proto.Schema_Object_LIST:
		out.Nesting = schema.NestingList
	case proto.Schema_Object_SET:
		out.Nesting = schema.NestingSet
	case proto.Schema_Object_MAP:
		out.Nesting = schema.NestingMap
	default:
		return nil, fmt.Errorf("unsupported nested attribute nesting %v", o.Nesting)
	}
	return out, nil
}

func nesting6(n proto.Schema_NestedBlock_NestingMode) (schema.NestingMode, error) {
	switch n {
